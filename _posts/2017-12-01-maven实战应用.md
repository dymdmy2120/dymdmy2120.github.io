...
其实在运行某个类的main的方法时会将src/main/java中的类文件放在当前项目下的target文件夹下，该文件夹中有 classes(存放主程序相关的class文件和配置文件)和test-classes(存放测试程序相关的类文件和测试用的配置文件)和打包好的jar或war包。其实只是运行main方法时，并不会将resources目录下的文件打包，只是简单的编译。只有当运行 mvn package时才会进行对resoures目录下的配置文件打包。其main/resources下的文件会放在 target/classes下面 test/resources下的文件放在 target/test-classes/下
...
1、compile 这个是默认的scope对应编译时、测试、运行（打包时能包含到war包中，jar包则不能必须使用对应的maven-assembly-plugin和maven-shade-plugin插件才可以将依赖包打包）
...
传递性依赖是和 pom.xml中<dependency>中的scope和 <parent>(表示该项目继承了父项目，下面会提到)有关联的。 例如 A项目依赖B项目，maven首先会根据坐标(groupId+artifactId+version)到本地找对应的jar，找到了则引入否则到中央仓库中获取。其中maven也会读取对应坐标下的pom文件，然后继续
...
>前言
>
>java项目中肯定少不了spring，其中spring运行又要加载spring的xml配置文件，其中配置文件头部会声明命名空间对应的xsd文件例如xsd文件 http://www.springframework.org/schema/aop/spring-aop-3.0.xsd ，所以需要找到对应的xsd文件。然后会根据当前的classpath下的META-INF牡蛎下找到对应的spring.schemas(里面包含了xsd文件对应存放位置，key为xsd文件 http://www.springframework.org/schema/aop/spring-aop-3.0.xsd value=META-INFA/spring-aop-3.0.xsd)，spring.handlers(key=xxx.xsd value=com.dynamo.xxxHandler)文件定义了将命名空间对应的元素以及属性转换成对应的对象的处理类
